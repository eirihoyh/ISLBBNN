...
for d in dependent_level:
    ...
    for ni in range(n_nets):
        ...
        if save_res:
            torch.save(all_nets[ni], f"network/lrt_class_high_sigma/net{ni}_unif_{round(d*100)}")
    ...
    if save_res:
        ...
        np.savetxt(f'results/high_sigma/lrt_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_{round(d*100)}_lin_func_early_stop_sigmoid_full.txt',m,delimiter = ',')
        m_median = np.array(metrics_median_several_runs)
        np.savetxt(f'results/high_sigma/lrt_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_{round(d*100)}_lin_func_early_stop_sigmoid_median.txt',m_median,delimiter = ',')
