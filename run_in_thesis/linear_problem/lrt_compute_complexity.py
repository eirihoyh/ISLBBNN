...
for d in dep_levels:
    ...
    complexity[f"dep: {d}"] = {}
    for i in range(10):
        net = torch.load(f"network/lrt_class_high_sigma/net{i}_unif_{d}", map_location=torch.device('cpu'))
...
# np.save("complexity/lrt_class/early_stop_non_lin_func/complexity", complexity)
# np.save("complexity/lrt_class/early_stop_non_lin_func/second_der", second_der)
# np.save("complexity/lrt_class/early_stop_non_lin_func/first_der", first_der)
