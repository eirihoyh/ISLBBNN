...
y, X = pip_func.create_data_unif(classification=True, non_lin=False)
...
class BayesianNetwork(nn.Module):
    def __init__(self):
        ...
        self.linears = nn.ModuleList([BayesianLinear(p, dim, a_prior=0.05, num_transforms=num_transforms)])
        self.linears.extend([BayesianLinear((dim+p), (dim), a_prior=0.05, num_transforms=num_transforms) for _ in range(HIDDEN_LAYERS-1)])
        self.linears.append(BayesianLinear((dim+p), 1, a_prior=0.05, num_transforms=num_transforms))
...
for d in dependent_level:
    ...
    y, X = pip_func.create_data_unif(classification=True, dep_level=d, non_lin=False)
    ...
    for ni in range(n_nets):
        ...
        for epoch in range(epochs):
            ...
            if ensemble_val > highest_acc and epoch >= 100:
                counter = 0
                # lowest_nll = nll_val
        ...
        if save_res:
            torch.save(all_nets[ni], f"network/flow_class_high_sigma/net{ni}_unif_{round(d*100)}_dep")
    ...
    if save_res:
        ...
        np.savetxt(f'results/high_sigma/flow_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_{num_transforms}_num_trans_{round(d*100)}_lin_func_early_stop_full.txt',m,delimiter = ',')
        m_median = np.array(metrics_median_several_runs)
        np.savetxt(f'results/high_sigma/flow_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_{num_transforms}_num_trans_{round(d*100)}_lin_func_early_stop_median.txt',m_median,delimiter = ',')
