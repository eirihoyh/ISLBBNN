...
class BayesianLinear(nn.Module):
    def __init__(self, in_features, out_features, num_transforms, lower_init_lambda=5, upper_init_lambda=15, a_prior=0.1):
        super().__init__()
        ...
        self.sigma_prior = (self.mu_prior + 2.5).to(DEVICE)
        ...
        self.bias_sigma_prior = (self.bias_mu_prior + 2.5).to(DEVICE)
    ...
    def kl_div(self):
        ...
        W_mean = z2 * self.weight_mu * self.alpha
        W_var = self.alpha * (self.weight_sigma ** 2 + (1 - self.alpha) * self.weight_mu ** 2 * z2 ** 2)
