...
for d in dependent_level:
    ...
    for ni in range(n_nets):
        ...
        if save_res:
            torch.save(all_nets[ni], f"network/flow_class/net{ni}_unif_dep_{round(d*100)}")
    ...
    if save_res:
        ...
        np.savetxt(f'results/flow_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_{lr}_lr_{num_transforms}_num_trans_non_lin_func_early_stop_dep_{round(d*100)}_full.txt',m,delimiter = ',')
        m_median = np.array(metrics_median_several_runs)
        np.savetxt(f'results/flow_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_{num_transforms}_num_trans_non_lin_func_early_stop_dep_{round(d*100)}_median.txt',m_median,delimiter = ',')
