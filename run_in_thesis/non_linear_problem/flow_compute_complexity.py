...
#     complexity[f"dep: {d}"] = {}
#     for i in range(10):
#         net = torch.load(f"network/flow_class/net{i}_unif_{d}_dep", map_location=torch.device('cpu'))
...
for d in dep_levels:
    ...
    complexity[f"dep: {d}"] = {}
    for i in range(10):
        net = torch.load(f"network/flow_class/net{i}_unif_dep_{d}", map_location=torch.device('cpu'))
        rand_numbs = torch.FloatTensor(100000, 4).uniform_(-10,10)
        first, second, combined_complexity = pip_func.complexity_measure(net, p, rand_numbs)
        first_der[f"dep: {d}"][i] = first
        second_der[f"dep: {d}"][i] = second
