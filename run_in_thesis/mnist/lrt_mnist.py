...
for BATCH_SIZE in BATCH_SIZE_LIST:
    ...
    for i in range(n_nets):
        ...
        if save_res:
            torch.save(net, f"network/lrt_class/net{i}_large_{BATCH_SIZE}_bs")
    ...
    if save_res:
        np.savetxt(f'results/lrt_class/MNIST_KL_loss_FLOW_{HIDDEN_LAYERS}_hidden_{dim}_dim_{lr}_lr_{num_transforms}_num_trans_LARGE_{BATCH_SIZE}_bs' + '.txt', loss_several_runs, delimiter=',')
        np.savetxt(f'results/lrt_class/MNIST_KL_metrics_FLOW_{HIDDEN_LAYERS}_hidden_{dim}_dim_{lr}_lr_{num_transforms}_num_trans_LARGE_{BATCH_SIZE}_bs' + '.txt', metrics_several_runs, delimiter=',')
        np.savetxt(f'results/lrt_class/MNIST_KL_nll_FLOW_{HIDDEN_LAYERS}_hidden_{dim}_dim_{lr}_lr_{num_transforms}_num_trans_LARGE_{BATCH_SIZE}_bs' + '.txt', nll_several_runs, delimiter=',')
