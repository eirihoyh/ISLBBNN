...
def create_data_unif(n=n_samples,beta=[100,1,1,1,1], dep_level=0,classification=False, dep=False):
    # Create data
    np.random.seed(42)
    ...
    if dep: 
        ...
        y = beta[0] + x1 + x2 + x1**2 + x2**2 + x1*x2 # non-linear model
...
def second_derivative_output_mnist(net,p,rand_numbs):
    second_der = {}
    for c in range(10):
        second_der[c] = {}
        for i in range(p):
...
def complexity_measure_mnist(net,p,rand_numbs):
    second = second_derivative_output_mnist(net, p, rand_numbs)
    mc_int_second = {}  # Second derviative "complexity"
...
def train(net,train_data, optimizer, batch_size, num_batches, p, DEVICE, nr_weights, val_data=None, flows=False, reg=False, epoch=1, multiclass=False):
    net.train()
    old_batch = 0
...
def val(net, val_data, DEVICE, flows=False, reg=False, verbose=True, multiclass=False):
    '''
    ...
    with torch.no_grad():
        ...
        if reg:
            ...
            outputs1 = outputs.T[0]
            a = metric(outputs1, target.T[0]).cpu().detach().numpy()
        else:
            if multiclass:
                output1 = outputs
...
def test_ensemble(net, test_data, DEVICE, SAMPLES, flows=False, reg=True, verbose=True, multiclass=False):
    net.eval()
    metr = []
    metr_median = []
    density = []
    ...
    with torch.no_grad():
        ...
        if reg:
            metric = R2Score().to(DEVICE)
            a_r2 = metric(outputs.T[0], target).cpu().detach().numpy()
            a_median_r2 = metric(outputs_median.T[0], target).cpu().detach().numpy()
            
            mean_squared_error = MeanSquaredError().to(DEVICE)
            a_mse = mean_squared_error(outputs.T[0], target).cpu().detach().numpy()
            a_median_mse = mean_squared_error(outputs_median.T[0], target).cpu().detach().numpy()
            
            mean_absolute_error = MeanAbsoluteError().to(DEVICE)
            a_mae = mean_absolute_error(outputs.T[0], target).cpu().detach().numpy()
            a_median_mae = mean_absolute_error(outputs_median.T[0], target).cpu().detach().numpy()

            a = np.array([a_r2, a_mse, a_mae])
            a_median = np.array([a_median_r2, a_median_mse, a_median_mae])
            ...
            if multiclass:
                ...
                # a_median = a_median.detach().numpy()
            else:
                output1 = outputs.T.mean(0)
                class_pred = output1.round().squeeze()
                a = np.mean((class_pred.cpu().detach().numpy() == target.cpu().detach().numpy().T[0]) * 1)
    ...
    if reg:
        metr.append(np.mean(ensemble, 0))
        metr_median.append(np.mean(ensemble_median, 0))
    else:
    ...
    if verbose:
        ...
        if reg:
            print(np.mean(ensemble,0), 'ensemble full')
            print(np.mean(ensemble_median, 0), 'ensemble median')
        else:
