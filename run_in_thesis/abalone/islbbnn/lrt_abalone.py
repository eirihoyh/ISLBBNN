...
for ni in range(n_nets):
    ...
    if save_res:
        torch.save(all_nets[ni], f"trained_nets/lrt/net{ni}")
    # Results
...
if save_res:
    ...
    np.savetxt(f'results/lrt_reg_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_abalone_full.txt',m,delimiter = ',')
    m_median = np.array(metrics_median_several_runs)
    np.savetxt(f'results/lrt_reg_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_abalone_median.txt',m_median,delimiter = ',')