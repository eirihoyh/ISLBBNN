...
class BayesianNetwork(nn.Module):
    def __init__(self):
        ...
        self.linears = nn.ModuleList([BayesianLinear(p, dim, a_prior=0.2, num_transforms=num_transforms)])
        self.linears.extend([BayesianLinear((dim+p), (dim), a_prior=0.2, num_transforms=num_transforms) for _ in range(HIDDEN_LAYERS-1)])
        self.linears.append(BayesianLinear((dim+p), CLASSES, a_prior=0.2, num_transforms=num_transforms))
    ...
    def forward(self, x, sample=False, ensemble=False):
        x_input = x.view(-1, p)
        x = F.relu(self.linears[0](x_input, ensemble))
        i = 1
        for l in self.linears[1:-1]:
            x = F.relu(l(torch.cat((x, x_input),1), ensemble))
            i += 1
...
for lr in lr_list:
    ...
    for ni in range(n_nets):
        ...
        if save_res:
            torch.save(all_nets[ni], f"network/flow_class/net{ni}_{lr}lr_relu")
    ...
    if save_res:
        ...
        np.savetxt(f'results/flow_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_fer2013_relu_full.txt',m,delimiter = ',')
        m_median = np.array(metrics_median_several_runs)
        np.savetxt(f'results/flow_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_fer2013_relu_median.txt',m_median,delimiter = ',')