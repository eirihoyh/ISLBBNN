...
for lr in lr_list:
    ...
    for ni in range(n_nets):
        ...
        if save_res:
            torch.save(all_nets[ni], f"network/flow_class/net{ni}_hsn_{lr}lr_relu")
    ...
    if save_res:
        ...
        np.savetxt(f'results/flow_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_fer2013_hsn_relu_full.txt',m,delimiter = ',')
        m_median = np.array(metrics_median_several_runs)
        np.savetxt(f'results/flow_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_fer2013_hsn_relu_median.txt',m_median,delimiter = ',')