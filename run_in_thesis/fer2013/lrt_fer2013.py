...
for ni in range(n_nets):
    ...
    for epoch in range(epochs):
        ...
        if ensemble_val >= highest_acc and epoch>1500:
            counter = 0
    ...
    if save_res:
        torch.save(all_nets[ni], f"network/lrt_class/net{ni}")
...
if save_res:
    ...
    np.savetxt(f'results/lrt_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_fer2013_full.txt',m,delimiter = ',')
    m_median = np.array(metrics_median_several_runs)
    np.savetxt(f'results/lrt_class_skip_{HIDDEN_LAYERS}_hidden_{dim}_dim_{epochs}_epochs_{lr}_lr_fer2013_median.txt',m_median,delimiter = ',')